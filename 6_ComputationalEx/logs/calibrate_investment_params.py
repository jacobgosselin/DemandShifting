...
sigma_init = struct_params["sigma"].iloc[0]   # initial guess only; sigma is calibrated
...
print("  sigma_init = {:.6f}  (initial guess; calibrated)".format(sigma_init))
...
alpha_a_init  = 0.5
alpha_k_init  = 0.5
sigma_init_cal = sigma_init   # from struct_params markup inversion
...
if os.path.isfile(out_path):
    ...
    if "sigma" in _cal_prev.columns:
        sigma_init_cal = float(_cal_prev["sigma"].iloc[0])
    ...
    print("  sigma_init_cal = {:.6f}".format(sigma_init_cal))
else:
...
def obj_base(x, max_nfev=30, vf_maxit=200):
    alpha_a_try, alpha_k_try, sigma_try = (
        float(x[0]), float(x[1]), float(x[2])
    )
    phi_try = phi_from_coef(coef_1980, sigma_try)
    ...
    res = np.array([
        ...
        (pct     - neg_ebitda_base_pct) / neg_ebitda_base_pct,
    ])
    if not np.all(np.isfinite(res)):
        return np.full(3, 1e3)

    print(
        "aa={:.4f} ak={:.4f} sig={:.4f} phi0={:.4f} | "
        "adv={:.4f} inv={:.4f} pct={:.2f}% | "
        "||res||={:.6f}".format(
            alpha_a_try, alpha_k_try, sigma_try, phi_try,
            adv_med, inv_med, pct,
            np.linalg.norm(res))
    )
...
_BOUNDS_LOW  = [0.1, 0.1,  2.0]   # alpha_a, alpha_k, sigma
_BOUNDS_HIGH = [0.9, 0.9, 20.0]   # alpha_a, alpha_k, sigma
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("Phase 1: Solve for alpha_a, alpha_k, sigma  (phi_0 backed out from coef_1980)")
    print("  Targets: adv/rev={:.4f}  capx/rev={:.4f}  pct_neg@1980={:.2f}%".format(
        med_sga_sale, med_capx_sale, neg_ebitda_base_pct))
    print("  coef_1980 = {:.6f}".format(coef_1980))
    print("=" * 60)
    ...
    result = minimize(
        obj_base_scalar,
        x0=[alpha_a_init, alpha_k_init, sigma_init_cal],
        method='Powell',
        bounds=list(zip(_BOUNDS_LOW, _BOUNDS_HIGH)),
        options=dict(xtol=1e-6, ftol=1e-6, maxiter=500, maxfev=5000),
    )

    alpha_a_cal = float(result.x[0])
    alpha_k_cal = float(result.x[1])
    sigma_cal   = float(result.x[2])
    phi_1980    = phi_from_coef(coef_1980, sigma_cal)
    ...
    print("  phi_1980     = {:.6f}  (backed out from coef_1980)".format(phi_1980))
    ...
    eqm_base = _solve_eqm(phi_1980, alpha_a_cal, alpha_k_cal, sigma_cal, fixed_cost=0.0)

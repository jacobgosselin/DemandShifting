...
# 8 core brackets present in all 39 post-1980 years
CORE_BRACKETS <- c(0, 25e3, 1e5, 5e5, 1e6, 5e6, 1e7, 5e7)

# assign every non-core bracket to the highest core threshold <= its thres_low
# (i.e. collapse intermediate splits into the core bin they belong to)
d[, core_bracket := CORE_BRACKETS[findInterval(thres_low, CORE_BRACKETS)]]

# sum firm counts within each core bracket × year
# use number_total from any row (it's the same for all rows within a year)
...
ggsave("figures/empirical/bracket_return_shares_all.pdf", pA,
       width = 8, height = 6)
...
ggsave("figures/empirical/bracket_return_shares_3group.pdf", pB,
       width = 8, height = 6)

# ---- Plot C: bottom vs. 2 middle vs. top, collapsed -------------------------
d_agg[, group_c := fcase(
...
pC <- ggplot(group_c_shares, aes(x = year, y = pct_returns, colour = group_c)) +
  ...
  geom_point(size = 3) +
  scale_colour_manual(
    values = c("< $25K" = "#2166ac", "$25K to $100K" = "lightgreen", "$100K to $500K" = "green", "$500K to $1M" = "darkgreen", "$1M+" = "#d73027"),
    name = NULL
  ) +
  labs(
    title = "",
...
ggsave("figures/empirical/bracket_return_shares_4group.pdf", pC,
       width = 8, height = 6)

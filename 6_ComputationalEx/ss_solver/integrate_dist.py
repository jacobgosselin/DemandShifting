...
def pct_negative_profit(m_grid, k_grid, z_grid, eqm):
    """Compute percent of firms with profit (earnings minus depreciation cost) < 0."""
    profit_pdf, _ = est_dist(m_grid, k_grid, z_grid, eqm, 'profit')
    mask = profit_pdf[:, 0] < 0
    total_mass = np.sum(profit_pdf[:, 1])
    negative_mass = np.sum(profit_pdf[mask, 1])
    return 100.0 * negative_mass / total_mass
